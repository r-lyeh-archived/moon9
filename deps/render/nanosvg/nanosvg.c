...
#define NANOSVG_M_PI 3.14159265358979323846264338327
...
#define NANOSVG_TAG 1
#define NANOSVG_CONTENT 2
#define NANOSVG_MAX_ATTRIBS 256
...
static void parseElement(char* s,
						 ...
						 void* ud)
{
	const char* attr[NANOSVG_MAX_ATTRIBS];
	int nattr = 0;
	char* name;
	int start = 0;
	int end = 0;
	...
	// Get attribs
	while (!end && *s && nattr < NANOSVG_MAX_ATTRIBS-1)
	{
			 ...
			 void* ud)
{
	char* s = input;
	char* mark = s;
	int state = NANOSVG_CONTENT;
	while (*s)
	{
		if (*s == '<' && state == NANOSVG_CONTENT)
		{
			// Start of a tag
			*s++ = '\0';
		...
		else if (*s == '>' && state == NANOSVG_TAG)
		{
			// Start of a content or new tag.
			*s++ = '\0';
			parseElement(mark, startelCb, endelCb, ud);
			mark = s;
			state = NANOSVG_CONTENT;
		}
		else
			s++;
	}
...
struct SVGParser
{
	struct SVGAttrib attr[NANOSVG_MAX_ATTR];
	int attrHead;
	float* buf;
	int nbuf;
	int cbuf;
...
{
	if (p->attrHead < NANOSVG_MAX_ATTR-1)
	{
		p->attrHead++;
...
static unsigned int parseColor(const char* str)
{
	...
	if (*str == '#')
    {
        ...
        switch( read )
        {
            case 3:
                sscanf(str + 1, "%x", &read);
                c = ((( read & 0xF00 ) >> 8 ) * 0x11 ) << 0 | ((( read & 0x0F0 ) >> 4 ) * 0x11 ) << 8 | ((( read & 0x00F ) >> 0 ) * 0x11 ) << 16 | 0xFF << 24;
                break;
            case 6:
            case 8:
                sscanf(str + 1, "%x", &c);
...
static void svgParseCircle(struct SVGParser* p, const char** attr)
{
	...
	if (r != 0.0f)
	{
		...
		n = (int)ceilf(NANOSVG_M_PI*2/da);

		da = (float)(NANOSVG_M_PI*2)/n;
		for (i = 0; i < n; ++i)
		{
			u = i*da;
			x = cx + cosf(u)*r;
