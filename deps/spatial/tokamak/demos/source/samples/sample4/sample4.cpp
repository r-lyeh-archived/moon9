...
void CSample4::Initialise() 
{

	CLuaValue &titleLUA = CLuaValue("Tokamak demo - Rad Dude - (c) 2003 Tokamak Ltd");
	...
	neV3 tmp; tmp.Set(-30,12,40);

	targetCam.cam->SetPos(tmp);

	targetCam.xDir = -0.76f;

	targetCam.yDir = -0.19f;
...
void CSample4::Process(OSTime time)
{
	...
	{
		vel.Set (0.0f,0.0f,5.0f);

		rigidBodies[actOnBody]->SetVelocity(vel);
	}
	else if (GETOS->IsKeyPress('1'))
	{
		vel.Set (0.0f,0.0f,-5.0f);
...
void CSample4::DisplayText()
{
	...
	GETDISPLAY->PrintText(0, 8, "Rag doll demonstration. This rag doll is constructed using");
	GETDISPLAY->PrintText(0, 9, "a combination of hinge and ball joints, with joint limits enabled.");
...
void CSample4::InititialisePhysics()
{
...
void CSample4::MakeRadDude(neV3 position, s32 & index)
{
...
void CSample4::InititialiseTokens()
{
	s32 curToken = 0;
