/*
 *	Modify from Mark Kilgard's glutdino GLUT sample
 *
 *	You can get the original sample from here
 *
...
extern void sampleDraw();
extern void sampleInit();
extern void sampleUpdate();
extern void sampleShutdown();
extern void sampleKeyFunc(unsigned char c, int x, int y);
extern char * SampleTitle;
    ...
    if (renderReflection) {
      if (stencilReflection) {
        /* We can eliminate the visual "artifact" of seeing the "flipped"
  	   object underneath the floor by using stencil.  The idea is
	   draw the floor without color or depth update but so that 
	   a stencil value of one is where the floor will be.  Later when
	   rendering the object reflection, we will only update pixels
...
enum {
  M_NONE, M_MOTION, M_LIGHT, M_TEXTURE, M_SHADOWS, M_REFLECTION, 
...
main(int argc, char **argv)
{
  ...
  glutCreateWindow(SampleTitle);

  if(0)
  if (glutGet(GLUT_WINDOW_STENCIL_SIZE) <= 1) {
    printf("Sorry, I need at least 2 bits of stencil (instead of %d)\n", glutGet(GLUT_WINDOW_STENCIL_SIZE));
    exit(1);
  }

  printf("Press 'P' to toggle pause\n");
  printf("Press 'Q' to lift rigid body\n");
  ...
  /* aspect ratio */ 640.0/400.0,
    /* Z near */ 20.0, /* Z far */ 1000.0);
  glMatrixMode(GL_MODELVIEW);
  gluLookAt(0.0, 8.0 * 0.8, 60.0 * 0.8,  /* eye is at (0,8,60) */
