...
// proposal:
// tween<type>( T &t, T &end, time );
// ie,
// tween<linear>( color, colorend, 3.0 );
...
//  extern
    inline float tween( tween_enum type, float current, float total, bool looped = false )
    {
        ...
        if( d <= 0.f || t <= 0.f )
        {
            return( type == COS2PI_11 || type == SINPI2PI_10 ? 1.f : 0.f );
        }
