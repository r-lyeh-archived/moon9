/* Simple tcp client sockets / MIT licensed
 * - rlyeh ~~ listening to Crippled Black Phoenix / We Forgotten Who We Are
 */

// ref [1]: http://beej.us/guide/bgnet/output/html/singlepage/bgnet.html#serialization
// ref [2]: from unpv12e/lib/connect_nonb.c
// ref [3]: some macros taken from SimpleSockets library
// ref [4]: http://tangentsoft.net/wskfaq/articles/bsd-compatibility.html
...
namespace tcp
{
    namespace
    {
        ...
        void exiting()
        {
            ...
            {
                fprintf( stdout, "<moon9/io/tcp.cpp> says: listening port %s is about to close", it->c_str() );

                // dummy request
                int dummyfd;
                tcp::connect( dummyfd, "127.0.0.1", *it );
            }
        }
