...
void Md5Skeleton::draw (const Matrix4x4f &modelView, bool labelJoints)
{
    ...
    if( labelJoints )
    {
        ...
        for( size_t i = 0; i < _joints.size (); ++i )
        {
            moon9::matrix::translate t( moon9::vec3(_joints[i]->pos._x, _joints[i]->pos._y, _joints[i]->pos._z) );
            moon9::text3d(_joints[i]->name, 0.0150f );
        }
...
void Md5Mesh::drawNormals () const
{
...
{
    _vertexArray.resize/*reserve*/ (_numVerts * 3);
    _normalArray.resize/*reserve*/ (_numVerts * 3);
    _tangentArray.resize/*reserve*/ (_numVerts * 3);
    _texCoordArray.resize/*reserve*/ (_numVerts * 2);
    _vertIndices.reserve (_numTris * 3);

    for( size_t i = 0; i < _numVerts; ++i )
      _vertexArray[i*3+0] = 0,
      _vertexArray[i*3+1] = 0,
      _vertexArray[i*3+2] = 0;

    for( size_t i = 0; i < _numVerts; ++i )
      _normalArray[i*3+0] = 0,
      _normalArray[i*3+1] = 0,
      _normalArray[i*3+2] = 0;

    for( size_t i = 0; i < _numVerts; ++i )
      _tangentArray[i*3+0] = 0,
      _tangentArray[i*3+1] = 0,
      _tangentArray[i*3+2] = 0;

    for( size_t i = 0; i < _numVerts; ++i )
      _texCoordArray[i*2+0] = 0,
      _texCoordArray[i*2+1] = 0;
...
void Md5Model::setMeshDecalMap( const string &name, const Texture2D *tex )
{
    if( Md5Mesh *mesh = getMeshByName(name) )
        mesh->setDecalMap (tex);
}

void Md5Model::setMeshSpecularMap( const string &name, const Texture2D *tex )
{
    if( Md5Mesh *mesh = getMeshByName(name) )
        mesh->setSpecularMap (tex);
}

void Md5Model::setMeshNormalMap( const string &name, const Texture2D *tex )
{
    if( Md5Mesh *mesh = getMeshByName(name) )
        mesh->setNormalMap (tex);
}

void Md5Model::setMeshHeightMap( const string &name, const Texture2D *tex )
{
    if( Md5Mesh *mesh = getMeshByName(name) )
...
const Md5Animation* Md5Model::anim( const string &name ) const
{
...
void Md5Animation::buildFrameSkeleton( vector<JointInfo> &jointInfos, vector<BaseFrameJoint> &baseFrame, vector<float> &animFrameData )
{
...
void Md5Animation::interpolate (int frameA, int frameB, float interp, Md5Skeleton *out) const
{
